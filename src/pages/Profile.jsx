...
import { User, Mail, Phone, MapPin, Package, Edit2, Save, Camera, ChevronRight, Clock } from 'lucide-react';
import { currentUser, orderHistory } from '../data/mockData';
...
const Profile = () => {
  ...
  return (
    <div className="pt-24 pb-20 px-6 max-w-7xl mx-auto">
      ...
      >
        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-r from-primary/10 to-accent/10 -z-10" />
                ...
                <form className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${isEditing ? 'bg-white border-primary/30 ring-4 ring-primary/5' : 'bg-slate-50 border-transparent'}`}>
                      ...
                      <input type="text" name="name" value={userData.name} onChange={handleChange} disabled={!isEditing} className="bg-transparent w-full outline-none text-slate-800 font-medium" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${isEditing ? 'bg-white border-primary/30 ring-4 ring-primary/5' : 'bg-slate-50 border-transparent'}`}>
                      ...
                      <input type="email" name="email" value={userData.email} onChange={handleChange} disabled={!isEditing} className="bg-transparent w-full outline-none text-slate-800 font-medium" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${isEditing ? 'bg-white border-primary/30 ring-4 ring-primary/5' : 'bg-slate-50 border-transparent'}`}>
                      ...
                      <input type="tel" name="phone" value={userData.phone} onChange={handleChange} disabled={!isEditing} className="bg-transparent w-full outline-none text-slate-800 font-medium" />
                    </div>
                  </div>
                </form>
              </motion.div>
